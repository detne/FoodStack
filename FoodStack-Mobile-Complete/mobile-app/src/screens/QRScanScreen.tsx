...
const QRScanScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const handleBarCodeScanned = async ({ type, data }: { type: string; data: string }) => {
    ...
    try {
      ...
      console.log('🔍 Scanning QR token:', qrToken);
      
      // Use our test QR token if scanning the test token
      const testToken = 'mobile-test-qr-123';
      const tokenToUse = qrToken === testToken ? testToken : qrToken;
      
      console.log('📡 Calling backend with token:', tokenToUse);
      
      // Call backend QR API
      const response = await fetch(`${getApiBaseUrl()}/api/v1/public/tables/${tokenToUse}`, {
        method: 'GET',
        headers: {
      ...
      if (!tableData.success) {
        throw new Error(tableData.message || 'Invalid QR code');
      }

      console.log('✅ Table info received:', tableData.data);
